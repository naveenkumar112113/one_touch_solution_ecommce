...
const PremiumProductCard = ({ product }) => {
    ...
    return (
        <div className="bg-white rounded-[1.5rem] p-4 shadow-sm border border-slate-100 hover:shadow-xl hover:border-brand-200 transition-all duration-300 group flex flex-col h-full relative">
            ...
            {/* Wishlist Button */}
            <button className="absolute top-4 right-4 z-10 w-8 h-8 bg-white/80 backdrop-blur rounded-full flex items-center justify-center text-slate-400 hover:text-red-500 hover:bg-red-50 transition-colors shadow-sm opacity-0 group-hover:opacity-100 focus:opacity-100">
                <FaRegHeart />
            </button>
